...
while IFS="" read -r envscript; do
    ...
    echo "# $envscript"
    cat "$envscript"
done >>"$ENVSH_TMP" < <("$HOME/bin/sh-pkg" helper find shell=any-posix,bash -- "$HOME/.sh-pkg/env.d/" -type f | sort)
...
while IFS="" read -r rcscript; do
    ...
    echo "# $rcscript"
    cat "$rcscript"
done >>"$RCSH_TMP" < <("$HOME/bin/sh-pkg" helper find shell=any-posix,bash -- "$HOME/.sh-pkg/rc.d/" -type f | sort)
